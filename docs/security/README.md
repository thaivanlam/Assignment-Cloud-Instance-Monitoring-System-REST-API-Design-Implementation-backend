...
| Document | Contents |
|---|---|
| [SECURITY_BUGS.md](SECURITY_BUGS.md) | 15 reproduced findings ranked by what an attacker gains, each with evidence and a fix; a **Status** column saying which are closed — SEC-04 and SEC-05, and SEC-08 in part; a suggested order of work; what was checked and found sound; the reproduction method |

## The short version

Everything in `SECURITY_BUGS.md` is a security defect, not a functional one — all 149
...
- **SEC-01** — the default `SECRET_KEY` is a literal in [config.py](../../app/config.py)
  ...
  sign a token. A forged `ADMIN` token was accepted at an `ADMIN`-only endpoint. `exp` is
  now required, but a forger simply sets one as far ahead as they like.
...
- **SEC-04** — *fixed.* There was no logout and no revocation; **SEC-08** was why — the
  token carried no `jti`. Tokens now carry a `jti` and `iat` (SEC-08, partly fixed —
  `iss` / `aud` are still absent), and `POST /api/auth/logout` denylists the token until it
  would have expired. Revoking every session of one member at once is still not possible.
- **SEC-05** — *fixed.* Login accepted unlimited guesses with no lockout and no log line.
  It now refuses a caller with `429` after 10 failures per account or 50 per address in 15
  minutes, before checking the password, and logs every failure. **SEC-09** pairs with it: a login against a known
...
of the document so any result can be re-run. SEC-04 and SEC-05 are fixed and SEC-08
partly — this document is the register, and the order to work through them is at
