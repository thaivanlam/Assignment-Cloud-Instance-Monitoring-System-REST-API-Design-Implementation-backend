...
| `204` | `DELETE /api/instances/{id}`, `POST /api/auth/logout` — no response body |
...
| `429` | `POST /api/auth/login` after too many failed attempts — carries `Retry-After` |

There is deliberately **no** `502`/`503` for LLM outages, nor for a Redis outage: the
rate limit, the token denylist and the diagnosis cache fail open
([AUTHENTICATION.md § 4](AUTHENTICATION.md#4-logout-and-revocation)). `GET /api/instances/{id}/diagnosis`
...
Raised directly in the auth controller, the auth dependencies and the alert controller:
...
| Too many failed logins for the account or the address | `429` | `Too many failed login attempts. Try again later.` — plus a `Retry-After` header |
| No `Authorization` header | `401` | `Not authenticated. Provide a Bearer token.` |
| Expired token | `401` | `Token has expired` |
| Malformed token, or no `exp` / `jti` claim | `401` | `Invalid token` |
...
|---|---|
| `POST /api/auth/login` | `401` bad credentials · `422` malformed email · `429` rate limited |
| `POST /api/auth/logout` | `401` missing, expired, invalid or already-revoked token |
